...
export const handler = schedule('0 * * * *', async (event) => {
    try {
        ...
        for (const [studentId, data] of Object.entries(notificationsToSend)) {
            ...
            const text = `Sevgili öğrenci, ${count > 1 ? `${count} adet ödevinin` : `"${titlesStr}" ödevinin`} teslimine son ${data.timeLeft} kaldı! Lütfen eksiklerini tamamla.`;
