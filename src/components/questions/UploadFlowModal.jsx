...
export default function UploadFlowModal({ onClose, studentId, studentName, showAlert }) {
    ...
    const handleFinalUpload = async () => {
        if (!questionImg || !selectedSubject) return;
        setStep(4);
        setQueueMessage('');
        ...
        const uploadToTelegram = async (base64, type) => {
            ...
            if (res.status === 429) {
                ...
                return uploadToTelegram(base64, type); // Retry
            }
        ...
        try {
            const qFileId = await uploadToTelegram(questionImg, 'question');
            let aFileId = null;
            if (answerImg) {
                aFileId = await uploadToTelegram(answerImg, 'answer');
            }

            await addDoc(collection(db, 'questions'), {
                studentId,
                folderId: selectedSubject,
            ...
            setStep(3); // Go back to subject selection on error
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/95 z-[9999] flex items-center justify-center p-4">
            {!isScanning ? (
                ...
                >
                    {step < 4 && (
                        <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-slate-100 text-slate-500 rounded-full hover:bg-slate-200 transition-colors z-10">
                            <X size={20} />
                        </button>
                    )}
                    ...
                    <div className="p-6 md:p-8">
                        {step === 1 && (
                            <div className="text-center">
                                <div className="w-16 h-16 bg-brandPurple/10 text-brandPurple rounded-2xl mx-auto flex items-center justify-center mb-6">
                                    <Camera size={32} />
                                </div>
                                ...
                                <label className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-sm bg-brandPurple text-white shadow-lg shadow-brandPurple/25 cursor-pointer hover:brightness-105 active:scale-95 transition-all">
                        ...
                        {step === 3 && (
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto pr-2 pb-2 custom-scrollbar">
                                    {SUBJECTS.map(sub => (
                                        <button 
                                            ...
                                            className={`p-3 rounded-xl font-bold text-sm text-left border-2 transition-all flex items-center justify-between ${selectedSubject === sub.id ? 'border-brandPurple bg-brandPurple/5 text-brandPurple' : 'border-slate-100 bg-slate-50 text-slate-600 hover:border-slate-200'}`}
                                        >
                                            {sub.name}
                                ...
                                <button 
                                    ...
                                    className={`w-full mt-6 py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-2 ${selectedSubject ? 'bg-brandPurple text-white shadow-lg shadow-brandPurple/25 hover:brightness-105' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
                                >
