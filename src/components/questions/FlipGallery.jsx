...
export default function FlipGallery({ questions, initialIndex = 0, onClose }) {
    ...
    const modalContent = (
        ...
        >
            {/* Topbar */}
            <div className="absolute top-0 left-0 w-full p-4 md:p-6 flex items-start justify-between z-[60]">
            ...
            <div className="relative w-full max-w-[380px] aspect-[3/4.2] flex items-center justify-center perspective-[1600px] mt-16 md:mt-10 z-[50]">
                {questions.map((q, index) => {
                    ...
                    const frontImg = imageUrls[q.questionImageId];
                    const backImg = q.answerImageId ? imageUrls[q.answerImageId] : null;
                    const cardIsFlipped = isCurrent ? isFlipped : false;

                    return (
                        <motion.div
                            key={index}
                            className={`absolute w-[85%] md:w-full h-full touch-none ${isCurrent ? 'cursor-grab active:cursor-grabbing' : 'pointer-events-none'}`}
                            initial={false}
                            animate={{
                                x: offset === 0 ? "0%" : offset > 0 ? "105%" : "-105%",
                                scale: isCurrent ? 1 : 0.88,
                                opacity: isCurrent ? 1 : 0.4,
                                rotateZ: isCurrent ? 0 : offset > 0 ? 3 : -3,
                            ...
                            dragElastic={0.8}
                            whileDrag={{ scale: 0.96, zIndex: 30 }}
                            onDragEnd={isCurrent ? handleDragEnd : undefined}
                            onClick={(e) => {
                                if (isCurrent && !e.defaultPrevented) setIsFlipped(!isFlipped);
                            }}
                        >
                            <motion.div 
                                className="absolute w-full h-full"
                                style={{ transformStyle: 'preserve-3d' }}
                                animate={{ rotateY: cardIsFlipped ? 180 : 0 }}
                            ...
                            >
                                ...
                                >
                                    <div className="flex-1 w-full relative p-2 bg-slate-50">
                                        {frontImg ? (
                                            <>
                                                <img src={frontImg} className="w-full h-full object-contain pointer-events-none rounded-[28px]" alt="Soru" />
                                                <button 
                                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); setFullscreenImage(frontImg); }}
                                ...
                                >
                                    <div className="flex-1 w-full relative p-2 bg-slate-50/50">
                                        {backImg ? (
                                            <>
                                                <img src={backImg} className="w-full h-full object-contain pointer-events-none rounded-[28px]" alt="Cevap" />
                                                <button 
                                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); setFullscreenImage(backImg); }}
