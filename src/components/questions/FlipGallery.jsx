...
export default function FlipGallery({ questions, initialIndex = 0, onClose }) {
    ...
    useEffect(() => {
        const loadImagesForCurrent = async () => {
            // Aggressively preload current, 2 previous, and 2 next slides
            ...
            <div className="relative w-full max-w-[380px] mt-16 md:mt-10 z-[50]">
                ...
                >
                    {questions.map((q, idx) => {
                        ...
                        return (
                            <SwiperSlide key={q.id || idx} virtualIndex={idx} className="rounded-[36px]">
                                <motion.div 
                                    className="absolute w-full h-full cursor-pointer"
                                    style={{ transformStyle: 'preserve-3d' }}
                                    animate={{ 
                                        rotateY: currentlyFlipped ? 180 : 0,
                                        z: currentlyFlipped ? 50 : 0, 
                                        scale: currentlyFlipped ? [1, 1.05, 1] : 1 
                                    }}
