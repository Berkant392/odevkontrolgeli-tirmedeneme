...
const LiveSessionManager = ({ classes = [], isTeacherMode = false, showAlert, loggedInStudent = null, startLiveSession, joinLiveSession }) => {
    ...
    const handleStudentJoin = (session) => {
        ...
        if (session.status === 'planned') {
            if (diffMs > 0) {
                // Not started yet
                const dateStr = start.toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' });
                showAlert('info', 'Ders Henüz Başlamadı', `Bu ders henüz başlamamıştır.\nBaşlangıç Zamanı: ${dateStr}`);
                return;
            } else {
                // Time has come but teacher has not clicked "Start"
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {visibleSessions.map(session => {
                                ...
                                const start = new Date(session.startTime);
                                const isTimeReached = isClassTimeReached(session.startTime);
