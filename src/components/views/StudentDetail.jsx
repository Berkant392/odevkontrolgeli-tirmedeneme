...
const StudentDetail = ({ selectedStudentForView, selectedClass, currentUserRole, activeTab, setActiveTab, isTeacherMode, openCellNoteModal, updateGrade, updateClassInDb, showAlert }) => {
    ...
    return (
        <motion.div key="student-detail" initial={{ opacity: 0, y: 30, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "spring", stiffness: 260, damping: 20 }} className={`${currentUserRole === 'vip-student' ? 'bg-[#FAF8F0] border-yellow-500/20 shadow-vip-glow' : 'bg-white border-slate-100 shadow-float'} rounded-2xl md:rounded-[2.5rem] p-3.5 md:p-10 border relative z-10`}>
            
            {isTeacherMode && (
                <div className={`flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 mb-6 md:mb-10 pb-5 md:pb-8 border-b ${currentUserRole === 'vip-student' ? 'border-yellow-500/20' : 'border-slate-100'} text-center md:text-left`}>
                    <motion.div whileHover={{ scale: 1.05, rotate: 3 }} className={`w-16 h-16 md:w-24 md:h-24 rounded-full flex items-center justify-center text-2xl md:text-4xl font-black shadow-xl shrink-0 border-2 md:border-4 ${currentUserRole === 'vip-student' ? 'real-gold-bg text-slate-900 border-white shadow-vip-glow' : 'bg-gradient-to-br from-smartBlue to-blue-500 text-deepNavy border-white shadow-sm'}`}>{selectedStudentForView.name.charAt(0)}</motion.div>
                    <div>
                        <h2 className={`text-xl md:text-5xl font-black mb-1.5 md:mb-2 tracking-tight flex items-center justify-center md:justify-start gap-2 text-slate-800`}>{selectedStudentForView.name}{currentUserRole === 'vip-student' && <Sparkle className="text-amber-500 w-4 h-4 md:w-5 md:h-5 animate-pulse"/>}</h2>
                        <div className="flex items-center justify-center md:justify-start gap-2 text-xs">
                            <span className={`font-bold px-2.5 py-0.5 rounded-md ${currentUserRole === 'vip-student' ? 'bg-yellow-500/10 border border-yellow-500/30 text-yellow-700' : 'bg-slate-100 text-slate-500'}`}>{selectedClass.className}</span>
                            <span className={`font-black px-2.5 py-0.5 rounded-md border ${currentUserRole === 'vip-student' ? 'bg-amber-100 text-amber-600 border-amber-200 shadow-sm' : 'bg-successGreen/10 text-successGreen border-successGreen/20'}`}>%{calculateStats([selectedStudentForView], selectedClass.topics).percentage} Başarı</span>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    {reversedTopics.map((topic, i) => {
                        ...
                        if (isOriginalOverdue) {
                            if (isAllDone) {
                                ...
                                cardColorStyle = currentUserRole === 'vip-student'
                                    ? 'bg-rose-50/60 border-rose-500/30 shadow-sm'
                                    : 'bg-rose-50/60 border-rose-100 shadow-sm';
                            }
                        } else {
                            cardColorStyle = currentUserRole === 'vip-student' 
                                ? 'bg-white border-yellow-500/20 shadow-sm' 
                        ...
                        return (
                            <motion.div key={topic.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`rounded-2xl md:rounded-3xl p-3.5 md:p-6 border transition-all duration-300 ${cardColorStyle}`}>
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 md:mb-6 gap-3 md:gap-4">
                                    <div className="flex items-center gap-2"><div className={`w-1.5 h-6 rounded-full ${theme.tag}`}></div><h3 className={`text-base md:text-xl font-black uppercase tracking-wide text-slate-800`}>{topic.title}</h3></div>
                                    <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-between md:justify-end">
                                            ...
                                            <div className={`text-[10px] md:text-xs font-bold px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-sm border ${
                                                isOriginalOverdue && !isAllDone 
                                                    ...
                                                    : isOriginalOverdue && isAllDone
                                                        ...
                                                        : (deadlineInfo.isToday ? 'bg-amber-500 text-white border-amber-600' : (currentUserRole === 'vip-student' ? 'bg-amber-50 text-amber-600 border-yellow-500/20' : 'bg-slate-50 text-slate-500 border-slate-200'))
                                            }`}>
                                        ...
                                        <div className="w-10 h-10 shrink-0">
                                            <CircularProgressbar 
                                                ...
                                                text={`%${pct}`} 
                                                styles={buildStyles({
                                                    pathColor: currentUserRole === 'vip-student' ? '#f59e0b' : '#0c3d64',
                                                    textColor: currentUserRole === 'vip-student' ? '#d97706' : '#0c3d64',
                                                    trailColor: currentUserRole === 'vip-student' ? '#fef3c7' : '#e2e8f0',
                                                    textSize: '28px',
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                                    {topic.subColumns?.map(col => {
                                        ...
                                        const cardStyle = currentUserRole === 'vip-student' ? `bg-white border-yellow-500/20 ${isMissed ? 'border-errorRed shadow-[0_0_12px_rgba(239,68,68,0.15)]' : ''}` : `bg-white border-slate-100 ${isMissed ? 'border-errorRed/40 shadow-[0_0_12px_rgba(239,68,68,0.15)]' : ''}`;
                                        const titleStyle = 'text-slate-800';
                                        return (
