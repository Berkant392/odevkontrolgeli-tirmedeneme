                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`p-4 ${selectedClass.type === 'vip' ? 'bg-yellow-50/30' : 'bg-slate-50/50'}`}>
                    {isMobile ? (
                        <div className="space-y-4">
                            ...
                            {selectedClass.students?.map((std) => ( <MobileStudentCard key={std.id} student={std} cls={selectedClass} updateGrade={updateGrade} onOpenNote={openCellNoteModal} onEditStudent={(s) => { setModalData({ classId: selectedClass.id, studentId: s.id, currentName: getSafeText(s.name) }); setModalInputVal(getSafeText(s.name)); setModalType('edit-student'); }} onDeleteStudent={deleteStudent} onPrintReport={handlePrintStudentReport} /> ))}
                        </div>
                    ) : (
            ...
            {activeTab === 'curriculum' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 bg-slate-50/50 border-t border-slate-100">
                    <CurriculumTracker 
                        ...
                        isTeacherMode={true} 
                        libraryItems={libraryItems ? libraryItems.filter(i => i.type === 'curriculum') : []} 
