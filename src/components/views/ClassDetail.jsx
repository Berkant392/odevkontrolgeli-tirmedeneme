...
import { Layout, Crown, Pencil, AlertOctagon, KeyRound, BookOpen, Plus, Trash2, Calendar, MoreVertical, UserPlus, Printer, Video } from 'lucide-react';
        ...
        <motion.div key="class-detail" initial={{ opacity: 0, y: 30, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "spring", stiffness: 260, damping: 20 }} className="bg-white rounded-[2rem] shadow-float border border-slate-200 overflow-hidden relative z-10">
            ...
            <div className={`p-4 md:p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-4 ${selectedClass.type === 'vip' ? 'bg-gradient-to-r from-yellow-50 to-white border-yellow-100' : 'bg-gradient-to-r from-slate-50 to-white border-slate-100'}`}>
                ...
                <div className="flex flex-wrap items-center gap-1.5 w-full md:w-auto justify-end">
                    ...
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => startLiveSession(selectedClass)} className={`text-[10px] md:text-xs text-white px-3 py-1.5 rounded-xl font-black shadow-md flex items-center gap-1 bg-gradient-to-tr from-brandPurple to-blue-600 hover:from-purple-700 hover:to-blue-700 shadow-glow`}><Video size={12}/> Canlı Ders</motion.button>
                    ...
                    {isMobile ? (
                        <div className="space-y-3.5">
                            ...
                            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-1.5">
                                <input type="text" placeholder="Yeni Öğrenci Adı..." className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 w-full focus:border-brandPurple outline-none font-medium" value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') addStudent(selectedClass.id); }} />
                                <div className="flex gap-1.5">
                                    <input type="text" placeholder="Tel (Opsiyonel)" className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 w-full focus:border-brandPurple outline-none font-medium" value={newStudentPhone || ''} onChange={(e) => {if(setNewStudentPhone) setNewStudentPhone(e.target.value);}} onKeyDown={(e) => { if(e.key === 'Enter') addStudent(selectedClass.id); }} />
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => addStudent(selectedClass.id)} className={`text-white px-3.5 rounded-xl text-xs font-black shadow-md ${selectedClass.type === 'vip' ? 'real-gold-bg text-slate-900' : 'bg-brandPurple'}`}>EKLE</motion.button>
                                </div>
                            </div>
                                                ...
                                                <div className="flex justify-between items-center group">
                                                    <div className="flex flex-col gap-1 cursor-pointer" onClick={() => openStudent(std)}>
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <button title="Düzenle" onClick={(e) => { e.stopPropagation(); handleEditStudentClick(std); }} className="text-slate-300 hover:text-brandPurple opacity-0 group-hover:opacity-100 transition-opacity"><Pencil size={14}/></button>
                                                        </div>
                                                    </div>
