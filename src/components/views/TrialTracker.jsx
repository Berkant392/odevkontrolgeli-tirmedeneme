...
const SubjectInput = ({ label, max, state, setState, color }) => {
    ...
    return (
        <div className={`p-3 md:p-4 rounded-2xl border ${color.border} ${color.bg} shadow-sm`}>
            <div className={`font-black text-[11px] md:text-xs uppercase tracking-widest ${color.text} mb-3`}>{label} <span className="opacity-60 text-[9px] lowercase ml-1">(Max: {max})</span></div>
            <div className="flex gap-2 md:gap-3">
                <div className="flex-[2]">
                    <label className="text-[9px] md:text-[10px] font-bold text-slate-500 block mb-1">Doğru</label>
                    <input type="text" inputMode="numeric" value={state.correct} onFocus={(e) => e.target.select()} onChange={(e) => handleNumberInput(e.target.value, max, 'wrong', 'correct', state, setState)} className="w-full bg-white border-2 border-slate-200 rounded-xl p-2 md:p-2.5 text-sm md:text-base font-black text-slate-700 outline-none focus:border-brandPurple transition-colors text-center" placeholder="0" />
                </div>
                <div className="flex-[2]">
                    <label className="text-[9px] md:text-[10px] font-bold text-slate-500 block mb-1">Yanlış</label>
                ...
                <div className="flex-[1.5] opacity-50">
                    <label className="text-[9px] md:text-[10px] font-bold text-slate-500 block mb-1 text-center">Boş</label>
                    <div className="w-full bg-slate-100 border-2 border-slate-100 rounded-xl p-2 md:p-2.5 text-sm md:text-base font-bold text-slate-500 text-center flex items-center justify-center min-h-[36px] md:min-h-[44px]">
                        {empty}
                    </div>
                </div>
                <div className="flex-[2] flex flex-col justify-end">
                    <div className="bg-white border-2 border-brandPurple/20 rounded-xl p-2 md:p-2.5 text-sm md:text-base font-black text-center text-brandPurple shadow-[0_0_15px_rgba(147,51,234,0.1)] flex items-center justify-center min-h-[36px] md:min-h-[44px]">
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="text-center mb-6">
                <h2 className={`text-2xl font-black mb-1 ${isVip ? 'text-white' : 'text-slate-800'}`}>Deneme Takip Paneli</h2>
                <p className="text-sm font-bold text-slate-400">Netlerini kaydet, gelişimini grafiklerle izle.</p>
            </div>

            <div className="grid grid-cols-3 gap-3 md:gap-4 mb-8">
                <div className={`p-4 md:p-6 rounded-3xl flex flex-col items-center justify-center text-center gap-2 ${isVip ? 'bg-slate-700 shadow-lg border border-slate-600' : 'bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100'}`}>
                    <div className="flex items-center gap-2">
                        <span className="text-xl md:text-3xl">📝</span>
                        <div className="text-xs md:text-sm font-bold text-slate-400">Toplam {activeFilter}</div>
                    </div>
                    <div className={`text-3xl md:text-4xl font-black ${isVip ? 'text-white' : 'text-slate-800'}`}>{stats.total}</div>
                </div>
                <div className={`p-4 md:p-6 rounded-3xl flex flex-col items-center justify-center text-center gap-2 ${isVip ? 'bg-slate-700 shadow-lg border border-slate-600' : 'bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100'}`}>
                    <div className="flex items-center gap-2">
                        <span className="text-xl md:text-3xl">🎯</span>
                        <div className="text-xs md:text-sm font-bold text-slate-400">Ortalama Net</div>
                    </div>
                    <div className={`text-3xl md:text-4xl font-black ${isVip ? 'text-white' : 'text-slate-800'}`}>{stats.avg}</div>
                </div>
                <div className={`p-4 md:p-6 rounded-3xl flex flex-col items-center justify-center text-center gap-2 ${isVip ? 'bg-slate-700 shadow-lg border border-slate-600' : 'bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100'}`}>
                    <div className="flex items-center gap-2">
                        <span className="text-xl md:text-3xl">🏆</span>
                        <div className="text-xs md:text-sm font-bold text-slate-400">En Yüksek Net</div>
                    </div>
                    <div className={`text-3xl md:text-4xl font-black ${isVip ? 'text-white' : 'text-slate-800'}`}>{stats.max}</div>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                <div className="flex bg-slate-100/80 p-1.5 rounded-2xl w-full md:w-auto">
                    {['TYT', 'AYT'].map(filter => (
                        <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-8 py-2.5 rounded-xl font-black text-sm transition-all ${activeFilter === filter ? 'bg-white text-brandPurple shadow-md' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'}`}>{filter}</button>
                    ))}
                </div>
                {!isTeacherMode && (
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { setExamType(activeFilter); setShowForm(true); }}
                        className={`w-full md:w-auto px-6 py-3 rounded-2xl font-black text-sm flex items-center justify-center gap-2 shadow-lg transition-all ${isVip ? 'real-gold-bg text-slate-900 hover:shadow-xl' : 'bg-brandPurple text-white hover:bg-purple-600 shadow-brandPurple/30'}`}>
                        <Edit3 size={18} strokeWidth={2.5} /> Yeni Deneme Gir
                    </motion.button>
                )}
            </div>

            {/* Grafik */}
            <div className={`p-5 md:p-8 rounded-3xl border ${isVip ? 'bg-slate-700 border-slate-600 shadow-lg' : 'bg-white border-slate-100 shadow-[0_8px_30px_-4px_rgba(0,0,0,0.05)]'}`}>
                <h3 className={`text-base font-black mb-8 flex items-center gap-2 ${isVip ? 'text-white' : 'text-slate-800'}`}>
            ...
            {displayTrials.length > 0 && (
                <div className="space-y-4">
                    ...
                    {[...displayTrials].reverse().map(trial => (
                        <div key={trial.id} className={`p-5 md:p-6 rounded-3xl border group relative overflow-hidden ${isVip ? 'bg-slate-700 border-slate-600 shadow-md' : 'bg-white border-slate-100 shadow-sm hover:shadow-md'} transition-all`}>
                            <div className="flex justify-between items-start mb-5">
                                <div>
                                    <span className={`text-[10px] font-black px-3 py-1 rounded-lg tracking-widest ${trial.type === 'TYT' ? 'bg-blue-50 text-blue-600' : 'bg-rose-50 text-rose-600'}`}>{trial.type}</span>
                                    <h4 className={`font-black text-xl mt-2 ${isVip ? 'text-white' : 'text-slate-800'}`}>{trial.title}</h4>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right">
                                        <span className={`text-2xl md:text-3xl font-black ${trial.type === 'TYT' ? 'text-blue-600' : 'text-rose-600'}`}>{trial.totalNet} <span className="text-sm opacity-50">Net</span></span>
                                    </div>
                                    {!isTeacherMode && (
                                        <button onClick={() => handleDelete(trial.id)} className="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={18} /></button>
                                    )}
                                </div>
                            </div>
                            <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4">
                                <div className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl ${isVip ? 'bg-slate-600 text-slate-300' : 'bg-slate-50 text-slate-500'}`}>
                                    <Calendar size={14} /> {formatDate(trial.date)}
                                </div>
                                {getDetailLabels(trial).map((d, i) => (
                                    <span key={i} className={`text-xs font-bold px-3 py-1.5 rounded-xl ${isVip ? 'bg-slate-600 text-slate-200' : 'bg-slate-50 text-slate-600'}`}>
                                        <span className="opacity-70 mr-1">{d.label}:</span> <span className="font-black text-[13px]">{typeof d.val === 'number' ? d.val.toFixed(2) : d.val}</span>
                                    </span>
                                ))}
                            </div>
                        </div>
            ...
            <AnimatePresence>
                {showForm && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                            <div className="p-5 md:p-6 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center shrink-0">
                                <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><Target className="text-brandPurple" size={20} /> Yeni Deneme Kaydı</h3>
                                <button onClick={() => setShowForm(false)} className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-full transition-colors"><X size={20} /></button>
                            </div>
                            
                            <div className="p-5 md:p-6 overflow-y-auto space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Deneme Adı</label>
                                        <input type="text" placeholder="Örn: 3D TG" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-3.5 font-bold text-sm text-slate-700 outline-none focus:bg-white focus:border-brandPurple transition-colors" />
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Tarih</label>
                                        <input type="date" value={formDate} onChange={(e) => setFormDate(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-3.5 font-bold text-sm text-slate-700 outline-none focus:bg-white focus:border-brandPurple transition-colors" />
                                    </div>
                                </div>
                                
                                <div>
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Sınav Türü</label>
                                    <div className="flex bg-slate-100 p-1.5 rounded-2xl">
                                        {['TYT', 'AYT'].map(type => (
                                            <button key={type} onClick={() => setExamType(type)} className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${examType === type ? 'bg-white text-brandPurple shadow-md' : 'text-slate-500 hover:text-slate-700'}`}>{type}</button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {examType === 'AYT' && <>
                                        <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 space-y-4">
                                            <h4 className="font-black text-sm text-slate-700 mb-2">Edebiyat / Sosyal-1</h4>
                            ...
                            <div className="p-5 md:p-6 bg-white border-t border-slate-100 flex gap-4 shrink-0">
                                <button onClick={() => setShowForm(false)} className="flex-1 py-4 bg-slate-50 text-slate-600 rounded-2xl font-black text-sm hover:bg-slate-100 transition-colors">İptal</button>
                                <button onClick={handleSave} className="flex-1 py-4 bg-brandPurple text-white rounded-2xl font-black text-sm hover:bg-purple-600 transition-colors shadow-glow flex items-center justify-center gap-2"><Save size={18}/> Kaydet</button>
                            </div>
                        </motion.div>
                    </div>
                )}
