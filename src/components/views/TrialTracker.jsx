                    ...
                    <div className="space-y-4 mb-8">
                        ...
                        {examType === 'AYT' && <>
                            <div className={`p-4 rounded-3xl border space-y-4 ${isVip ? 'bg-slate-700/50 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
                                ...
                                <SubjectInput label="Edebiyat" max={24} state={edebiyat} setState={setEdebiyat} color={{ bg: isVip ? 'bg-slate-800' : 'bg-white', border: 'border-rose-100', text: 'text-rose-600' }} />
                                <SubjectInput label="Tarih-1" max={10} state={tarih1} setState={setTarih1} color={{ bg: isVip ? 'bg-slate-800' : 'bg-white', border: 'border-orange-100', text: 'text-orange-600' }} />
                                <SubjectInput label="Coğrafya-1" max={6} state={cografya1} setState={setCografya1} color={{ bg: isVip ? 'bg-slate-800' : 'bg-white', border: 'border-amber-100', text: 'text-amber-600' }} />
                            </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-7xl mx-auto">
            ...
            {!isTeacherMode && (
                <motion.button
                    ...
                    className={`fixed bottom-24 right-5 z-[45] md:hidden w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all ${isVip ? 'real-gold-bg text-slate-900 shadow-[0_4px_20px_rgba(245,158,11,0.4)]' : 'bg-brandPurple text-white shadow-[0_4px_20px_rgba(139,92,246,0.4)]'}`}
                >
