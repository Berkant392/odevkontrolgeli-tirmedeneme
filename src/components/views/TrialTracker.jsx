...
import { Target, TrendingUp, Trash2, Plus, CheckCircle, Flame, Save, X, Calendar, Rocket, Trophy, Edit3, ChevronDown, ChevronUp } from 'lucide-react';
...
const TrialTracker = ({ studentId, isTeacherMode, showAlert, currentUserRole }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-7xl mx-auto">
                ...
                {showForm && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 md:p-4 bg-slate-900/60 backdrop-blur-md">
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white w-full max-w-2xl rounded-3xl md:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                            ...
                            <div className="p-4 md:p-6 overflow-y-auto space-y-4 md:space-y-6 overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
            ...
            {/* 📱 MOBİL FAB BUTON — Sol alt köşe */}
            {!isTeacherMode && (
                <motion.button
                    whileTap={{ scale: 0.9 }}
                    onClick={() => { setExamType(activeFilter); setShowForm(true); }}
                    className={`fixed bottom-24 left-5 z-[45] md:hidden w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all ${isVip ? 'real-gold-bg text-slate-900 shadow-[0_4px_20px_rgba(245,158,11,0.4)]' : 'bg-brandPurple text-white shadow-[0_4px_20px_rgba(139,92,246,0.4)]'}`}
                >
