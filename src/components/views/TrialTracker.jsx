    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className={`p-4 rounded-2xl md:rounded-3xl border mb-6 flex flex-col md:flex-row items-center justify-between gap-4 ${isVip ? 'bg-slate-700 border-slate-600' : 'bg-white border-slate-100 shadow-sm'}`}>
                ...
                <div className="flex items-center gap-2 w-full md:w-auto shrink-0 bg-brandPurple/5 p-1.5 md:p-2 rounded-2xl border border-brandPurple/10">
                    <Target size={18} className="text-brandPurple ml-1" />
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-brandPurple/70 uppercase px-1 leading-none">
                        ...
                        <div className="flex items-center gap-1 mt-1">
                            <input 
                                ...
                                className={`w-20 px-2 py-1.5 rounded-xl font-black text-sm outline-none border-2 transition-all ${isVip ? 'bg-slate-600 border-slate-500 text-white focus:border-amber-400' : 'bg-white border-transparent text-slate-700 focus:border-brandPurple'}`}
                            />
                            <button 
                                onClick={handleTargetSave}
                                className={`px-3 py-1.5 rounded-xl font-black text-xs transition-colors shadow-sm ${isVip ? 'bg-amber-400 text-slate-900 hover:bg-amber-300' : 'bg-brandPurple text-white hover:bg-purple-600'}`}
                            >
                                Kaydet
                            </button>
                        </div>
