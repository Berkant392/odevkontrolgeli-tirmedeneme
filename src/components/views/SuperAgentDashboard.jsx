...
const SuperAgentDashboard = ({ classes, allTrials }) => {
  ...
  "version": "1.0",
  "description": "Jarvis için gerçek tarayıcı sekmelerini kontrol eder ve ekran görüntüleri alır.",
  ...
  "content_scripts": [
    {
      "matches": ["*://localhost/*", "*://*.netlify.app/*", "*://odevtakip-145f5.web.app/*"],
      "js": ["content.js"]
    }
  ]
}`,
...
const injectJarvisCursor = () => {
    ...
    if (!cursor) {
        ...
        cursor.style.transition = 'all 0.8s cubic-bezier(0.19, 1, 0.22, 1)';
        cursor.style.top = '50%';
        cursor.style.left = '50%';
        
        cursor.innerHTML = \`
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" style="filter: drop-shadow(0 0 6px rgba(236, 72, 153, 0.8));">
            ...
            <div style="position: absolute; top: 0; left: 0; width: 8px; height: 8px; border-radius: 50%; background: #ec4899; box-shadow: 0 0 12px #ec4899; animation: jarvis-ping 1.5s infinite;"></div>
        \`;
...
// Mouse'u organik veya belirli koordinatlara hareket ettir
...
const startCursorMovement = () => {
    ...
    cursorInterval = setInterval(moveCursorOrganically, 1200);
};
...
window.addEventListener('message', (event) => {
    ...
    if (event.data && event.data.type === 'JARVIS_APP_REQUEST') {
        ...
        if (action === 'START_AGENT') {
            startCursorMovement();
            window.postMessage({ type: 'JARVIS_EXTENSION_RESPONSE', requestId, result: 'Ajan Başlatıldı' }, '*');
            return;
        }
        if (action === 'STOP_AGENT') {
            stopCursorMovement();
            window.postMessage({ type: 'JARVIS_EXTENSION_RESPONSE', requestId, result: 'Ajan Durduruldu' }, '*');
            return;
        }

        // Background script'e eylemi gönder
...
// Sayfa yüklendiğinde imleci göster
injectJarvisCursor();
