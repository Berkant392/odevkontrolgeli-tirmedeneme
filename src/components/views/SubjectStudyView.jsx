...
import { Check, Edit3, X, Plus, Trash2, LayoutGrid, Hexagon, Settings } from 'lucide-react';
...
const SubjectStudyView = ({ studentId, isTeacherMode, showAlert }) => {
    ...
    const handleAddSingleTopic = (title) => {
        updateGlobal({ id: `t_${Date.now()}`, title, order: currentList.length + 1, subtopics: [] }, 'add_topic');
    };

    const handleAddBulkTopics = (parsedTopics) => {
        const payload = parsedTopics.map((pt, idx) => ({
            id: `t_${Date.now()}_${idx}`,
            title: pt.title,
            order: currentList.length + 1 + idx,
    ...
    return (
        <div className="honey-quest-container">
            ...
            <section className="top-card">
                ...
                <div className="flex gap-2 mt-3 flex-wrap w-full">
                    {filteredSubjects.map(su => (
                        <button 
                            key={su.id}
                            onClick={() => setSelectedSubject(su.id)}
            ...
            <section className="quest flex-1 h-0 flex justify-center mt-[14px]">
                <div className="quest-scroll w-full h-full overflow-x-hidden overflow-y-auto pb-[120px] hide-scrollbar" style={{ overscrollBehaviorY: 'contain', WebkitOverflowScrolling: 'touch' }}>
