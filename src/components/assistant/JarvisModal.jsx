...
4. SANAL FARE VE ARAYÜZ KONTROLÜ (AJAN ROLÜ): Kullanıcı tarayıcıda bir işlem yapmanı istediğinde KESİNLİKLE "simulate_app_action" fonksiyonunu uygun action_type ve target ile çağırarak sanal fareyi o hedefe yönlendir. Karmaşık ve çok adımlı görevlerde (örn: 15-20 tıklamalı form doldurma işlemleri) bu fonksiyonu görev tamamen bitene kadar arka arkaya zincirleme (sequential) olarak çağırmaya devam et. Asla yarıda kesme. Hedef adlarını (target) belirlerken butonun veya alanın üzerindeki metni BİREBİR veya kısmen (örn: "Ödev Ekle" veya "Kaydet") kullan.
...
6. ÖDEV İŞARETLEME VE KAYDETME: Ödevleri işaretlemek için "mark_homework" çağır. İşlem bitince "save_and_close" çağır.
...
8. GÖREV KUYRUĞU VE ReAct DÖNGÜSÜ (ÇOK ÖNEMLİ): Kullanıcı sana aynı anda birden fazla görev verirse (Örn: "Ahmet'i aç, ödevine 100 gir, kaydet, sonra Ayşe'yi aç..."), bu görevleri içsel düşüncende bir sıraya koy (Task Queue). Her işlemi TEK TEK "simulate_app_action" ile gerçekleştir. Her işlemden sonra {success: true} yanıtı aldığında, görev listendeki bir sonraki hedefe geç. Tüm görevler bitene kadar durma! Asla tüm istekleri tek seferde yapmaya çalışma, her fonksiyondan dönen sonucu bekleyerek "Reasoning and Acting (ReAct)" mantığıyla ilerle.
9. PROAKTİF İÇERİK OLUŞTURMA (AUTO-CREATION): Eğer kullanıcı senden ekranda VAR OLMAYAN bir şeyi eklemeni isterse (Örn: "Trigonometri ödevi ekle" dedi ama ekranda "Trigonometri" yazmıyor), hedefe "Trigonometri" diyerek TIKLAMAYA ÇALIŞMA (hata alırsın). Bunun yerine proaktif ol: Gerekli Input alanını (Örn: "Yeni Ana Konu Başlığı") bulup oraya "type" eylemi ile "Trigonometri" metnini KENDİN YAZ ve ardından "EKLE" veya "KAYDET" butonuna tıkla. Olmayan şeyi arama, SEN OLUŞTUR!
...
- Fonksiyon çağırırken kullanıcıya teknik detay verme, sadece "Hemen yapıyorum" vb. de.`;
            ...
            const tools = [
                ...
                {
                    name: "simulate_app_action",
                    description: "Tarayıcı/arayüz üzerinde sanal fare kontrolü ile butonlara tıklama, metin yazma, kaydırma veya sayfa geçişleri yapar (Ajan rolü).",
                    parameters: {
                        type: "OBJECT",
                        properties: {
                            action_type: {
                                type: "STRING",
                                description: "Yapılacak sanal eylem tipi.",
                            ...
                            text: {
                                type: "STRING",
                                description: "Type (yazma) eylemi için girilecek metin."
                            }
                        },
                ...
                try {
                    if (name === "apply_system_action") {
                        ...
                        if (window.bhAgent && window.bhAgent.trigger) {
                            ...
                            const result = await window.bhAgent.trigger(args.action_type, args.target, args.text);
                            ...
                            if (result && result.success) {
                                response = { success: true, message: `Sanal fare eylemi (${args.action_type} -> ${args.target}) başarıyla tamamlandı.` };
                            } else {
                                response = { success: false, message: `Hata: Hedef '${args.target}' bulunamadı veya etkileşim kurulamadı.` };
                            }
                        } else {
                            response = { success: false, message: "Hata: Sanal fare arayüz motoru (window.bhAgent) aktif değil." };
                        }
                    }
                } catch (e) {
