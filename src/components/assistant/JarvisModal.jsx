...
const AssistantModal = ({ classes, updateClassInDb, onClose, initialStudent }) => {
    ...
    const startListening = useCallback(async () => {
        ...
        if (!geminiServiceRef.current) {
            ...
            Bir şeyi anlamadığında "Hocam tam anlayamadım, hangi öğrenci veya kaynak?" gibi kısa sorular sor.
            ...
            const tools = [
                {
                    name: "search_and_select_student",
                    description: "Kullanıcı bir öğrenci adı söylediğinde onu bulmak ve profiline girmek için kullan. Eğer öğrenciyi bulursan sana onun bilgilerini veya birden fazla varsa sınıf seçeneklerini döndüreceğim.",
                    parameters: {
                        type: "OBJECT",
                        properties: {
                            student_name: { type: "STRING", description: "Aranacak öğrencinin adı ve soyadı." }
                        },
                ...
                {
                    name: "mark_homework",
                    description: "Öğrencinin ödevini (örneğin 'Türev testini yapıldı yap' veya '1. konunun tüm kaynaklarını eksik yap') işaretlemek için kullanılır. Seçili öğrenci varsa çalışır.",
                    parameters: {
                        type: "OBJECT",
                        properties: {
                            topic_name: { type: "STRING", description: "Ödevin konusu (Örn: Türev, 1. Konu, Limit)." },
                            source_name: { type: "STRING", description: "Kaynak adı (Örn: Soru Bankası, Tümü, Föy 1). Belirtilmemişse boş bırak." },
                            status: { type: "STRING", description: "Ödevin durumu.", enum: ["yapıldı", "yapılmadı", "eksik"] }
                        },
                ...
                },
                {
                    ...
                    description: "Seçili öğrencinin durumu, deneme netleri, ödev yüzdesi veya müfredat hakkında bilgi/rapor istenildiğinde çağır.",
                    parameters: {
                        type: "OBJECT",
                        properties: {
                            student_name: { type: "STRING", description: "Raporu istenen öğrencinin adı. Söylenmediyse boş bırak." }
                        }
                    }
                }
            ];
            ...
            geminiServiceRef.current.onFunctionCall = async (call) => {
                ...
                try {
                    if (name === "search_and_select_student") {
                        // Basit metin eşleştirme ile öğrenci bul (JarvisModal'daki findStudentsAdvanced benzeri)
                        const searchNorm = turkishNormalize(args.student_name);
                        const matched = state.allStudents.filter(s => 
                            turkishNormalize(s.name).includes(searchNorm) || searchNorm.includes(turkishNormalize(s.name))
                        );

                        if (matched.length === 0) {
                            response = { success: false, message: "Veritabanında böyle bir öğrenci bulunamadı." };
                        } else if (matched.length === 1) {
                            const student = matched[0];
                            const targetClass = state.classes.find(c => c.id === student.classId);
                            // State güncellemelerini tetikle
                            setSelectedStudent(student);
                            setFoundStudents([student]);
                            setFoundTopics(targetClass?.topics || []);
                            setCommandMode('homework');
                            response = { success: true, message: `Öğrenci bulundu ve aktif edildi: ${student.name} (${targetClass?.className}). Kullanıcıya işlemi başardığını sesli onayla.` };
                        } else {
                            const names = matched.map(s => {
                                const tc = state.classes.find(c => c.id === s.classId);
                                return `${s.name} (${tc?.className || 'Bilinmiyor'})`;
                            });
                            response = { success: false, message: `Birden fazla eşleşen öğrenci bulundu: ${names.join(', ')}. Kullanıcıya bunlardan hangisini kastettiğini sözlü olarak sor.` };
                        }
                    } 
                    else if (name === "mark_homework") {
                        if (!state.selectedStudent) {
                            response = { success: false, message: "Şu an seçili bir öğrenci yok. Önce hangi öğrenci olduğunu sor." };
                        } else {
                            const targetClass = state.classes.find(c => c.id === state.selectedStudent.classId);
                            const topics = targetClass?.topics || [];
                            ...
                            // Sayısal konu eşleştirme (Örn: 1. Konu)
                            const topicOrderMatch = args.topic_name.match(/(\d+)/);
                            ...
                            if (!targetTopic) {
                                ...
                                if (columnsToUpdate.length === 0) {
                                    ...
                                    columnsToUpdate.forEach(col => {
                                        handleDraftGradeChangeRef.current?.(state.selectedStudent.id, targetTopic.id, col.id, args.status);
                                    });
                        ...
                        const studentToReport = state.selectedStudent;
                        if (!studentToReport) {
                            response = { success: false, message: "Şu an seçili bir öğrenci yok. Hangi öğrenci için rapor istediğini sor." };
                        } else {
                            const targetClass = state.classes.find(c => c.id === studentToReport.classId);
                            // Ham verileri topla
                            const examScores = studentToReport.exams || [];
                            const homeworkGrades = studentToReport.grades || {};
                            const totalTopics = targetClass?.topics?.length || 0;
                            
                            response = { 
                                success: true, 
                                message: "Ham öğrenci verisi ektedir. Bunu öğretmen için kısa, öz ve motive edici bir rapora dönüştürüp sesli söyle.", 
                                data: {
                                    name: studentToReport.name,
                                    class: targetClass?.className,
                                    isVip: studentToReport.isVip,
                                    total_exams_taken: examScores.length,
                                    last_exam_score: examScores.length > 0 ? examScores[examScores.length - 1].net : null,
                                    total_topics_in_curriculum: totalTopics,
                                    homework_data: homeworkGrades
                                }
                            };
                        }
                    }
