...
const VipDashboard = ({ vipClasses, onOpenClass, onNewVipClass }) => {
    ...
    return (
        <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between bg-[#FAF8F0] p-6 rounded-[2rem] shadow-vip-glow border border-yellow-500/20 relative">
                ...
                <div className="flex-1 w-full relative z-30">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-bold text-slate-700 outline-none focus:border-amber-500 transition-all shadow-sm placeholder:text-slate-400"
                        value={searchQuery}
                ...
                <form onSubmit={handleAddVipSubmit} className="flex-1 w-full flex gap-3 relative z-10">
                    <input
                        ...
                        className="flex-1 bg-white border border-yellow-500/20 rounded-2xl px-4 py-3.5 text-sm font-bold text-slate-700 outline-none focus:border-amber-500 transition-all placeholder:text-slate-400 shadow-sm"
                        value={vipName}
            ...
            <div className="space-y-4 mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    {vipClasses.map(cls => (
                        <motion.div key={cls.id} whileHover={{ y: -4, scale: 1.01 }} onClick={() => onOpenClass(cls)} className="bg-white p-6 rounded-3xl border border-yellow-500/20 shadow-sm hover:shadow-vip-glow transition-all cursor-pointer flex flex-col justify-between min-h-[160px] group relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start relative z-10">
                                ...
                                <span className="text-xs font-black text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg border border-yellow-500/20">
                                    VIP PORTAL
                                </span>
                            </div>
                            <div className="mt-5 relative z-10">
                                <h4 className="font-black text-slate-800 text-xl group-hover:text-amber-600 transition-colors truncate">
                                    {cls.className}
                                </h4>
                ...
                {vipClasses.length === 0 && (
                    <div className="text-sm font-bold text-slate-400 bg-white p-8 rounded-3xl border border-dashed border-yellow-500/20 text-center flex flex-col items-center justify-center gap-3 shadow-sm">
