...
import { Target, AlertCircle, Clock, BookOpen, TrendingUp, ChevronRight, Video, Sparkles } from 'lucide-react';
import StudentTrialChart from '../analytics/StudentTrialChart';
import CountdownTimer from '../ui/Countdown';
import { getGreetingMessage } from '../../utils/greetings';

const StudentDashboard = ({ classes, currentUserRole, loggedInStudent, onOpenClass, setView, countdownConfig, liveSessions = [], joinLiveSession, selectedClass }) => {
    ...
    // Aktif canlı ders oturumunu hesapla
    const activeStudentSession = Array.isArray(liveSessions)
        ? liveSessions.find(s => s.status === 'active' && studentClasses.some(cls => String(cls.id) === String(s.classId)))
        : null;

    // Planlanan yaklaşan canlı dersleri hesapla
    const plannedStudentSessions = Array.isArray(liveSessions)
        ? liveSessions.filter(s => s.status === 'planned' && studentClasses.some(cls => String(cls.id) === String(s.classId)))
        : [];
    ...
    return (
        ...
        >
            ...
            <AnimatePresence>
                {activeStudentSession && (
                    <motion.div
                        ...
                        className={`p-5 rounded-[2rem] border relative overflow-hidden shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4 transition-all duration-300 ${
                            isVip 
                                ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-yellow-500/30 shadow-[0_0_30px_rgba(245,158,11,0.25)]' 
                                : 'bg-gradient-to-br from-brandPurple via-purple-900 to-indigo-900 border-purple-500/30 shadow-[0_0_30px_rgba(124,58,237,0.25)]'
                        }`}
                    >
                        {/* Glow effect */}
                        <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full animate-[shimmer_3s_infinite] pointer-events-none"></div>
                        <div className="absolute -top-12 -left-12 w-32 h-32 bg-rose-500/10 rounded-full blur-3xl pointer-events-none"></div>
                        
                        <div className="flex items-center gap-4 relative z-10 w-full md:w-auto">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 relative ${
                                isVip ? 'bg-amber-500/20 text-vipGold' : 'bg-white/20 text-white'
                            }`}>
                            ...
                            <div className="min-w-0">
                                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border ${
                                    isVip ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-white/10 text-white border-white/20'
                                }`}>
                                    YAYINDA
                                </span>
                                <h2 className={`text-base md:text-xl font-black mt-1 leading-tight ${isVip ? 'real-gold-text' : 'text-white'}`}>
                                    Canlı Dersiniz Başladı!
                                </h2>
                                <p className={`text-xs md:text-sm font-medium ${isVip ? 'text-slate-400' : 'text-white/80'} mt-0.5`}>
                                    {activeStudentSession.className} grubu için görüntülü sınıf şu an aktif.
                                </p>
                            </div>
                        </div>
                        ...
                        <motion.button
                            ...
                            className={`w-full md:w-auto px-6 py-3 rounded-xl font-black text-sm transition-all duration-300 shadow-lg relative z-10 flex items-center justify-center gap-2 ${
                                isVip 
                                    ? 'bg-gradient-to-r from-amber-300 to-yellow-500 text-slate-900 hover:shadow-[0_0_20px_rgba(245,158,11,0.4)]' 
                                    : 'bg-white text-brandPurple hover:bg-slate-50 hover:shadow-[0_0_20px_rgba(255,255,255,0.4)]'
                            }`}
                        >
            ...
            {/* 🌟 PLANLANAN CANLI DERSLER HIZLI TAKİP LİSTESİ */}
            <AnimatePresence>
                {plannedStudentSessions.length > 0 && (
                    <motion.div
                        initial={{ opacity: 0, y: 15 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 15 }}
                        className={`rounded-3xl p-4 md:p-5 shadow-sm overflow-hidden relative border ${
                            isVip ? 'bg-slate-800/80 border-slate-700' : 'bg-white border-slate-100 shadow-sm'
                        }`}
                    >
                        <h3 className={`text-xs md:text-sm font-black uppercase tracking-widest flex items-center gap-2 mb-3.5 ${
                            isVip ? 'text-amber-400' : 'text-brandPurple'
                        }`}>
                            <Video size={16} className="animate-pulse" />
                            Yaklaşan Canlı Derslerim ({plannedStudentSessions.length})
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {plannedStudentSessions.map(session => {
                                const start = new Date(session.startTime);
                                const isTimeReached = start <= new Date();
                                const formattedTime = start.toLocaleString('tr-TR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' });
                                ...
                                return (
                                    <div 
                                        ...
                                        onClick={() => {
                                            if (!isTimeReached) {
                                                alert(`Bu canlı ders henüz başlamadı.\nBaşlangıç: ${formattedTime}\nLütfen ders saatinde bu panelden derse katılın.`);
                                            } else {
                                                alert("Ders saati geldi! Öğretmeninizin derse girişi aktifleştirmesi bekleniyor. Lütfen bekleyin...");
                                            }
                                        }}
                                    ...
                                    >
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                                            isVip ? 'bg-amber-500/10 text-amber-400' : 'bg-purple-100 text-brandPurple'
                                        }`}>
                                            <Calendar size={18} />
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <h4 className={`text-xs font-black truncate ${isVip ? 'text-white' : 'text-slate-800'}`}>
                                                {session.className} Canlı Sınıfı
                                            </h4>
                                            <p className={`text-[10px] mt-0.5 truncate ${isVip ? 'text-slate-400' : 'text-slate-500'}`}>
                                                {formattedTime} • {session.duration} dk
                                            </p>
                                        </div>
                                        
                                        <span className={`text-[9px] font-black px-2.5 py-1 rounded-lg shrink-0 ${
                                            isTimeReached
                                                ? 'bg-amber-500 text-white animate-pulse'
                                                : isVip ? 'bg-slate-800 text-slate-400 border border-slate-700' : 'bg-slate-200 text-slate-500'
                                        }`}>
                                            {isTimeReached ? 'HOCA BEKLENİYOR' : 'PLANLANDI'}
                                        </span>
                                    </div>
                                );
                            })}
