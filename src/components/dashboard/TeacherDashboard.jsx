        ...
        <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between bg-white p-5 rounded-[2rem] shadow-float border border-slate-100">
                ...
                <div className="flex-1 relative max-w-xl">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-bold text-slate-700 outline-none focus:border-deepNavy transition-all shadow-sm"
                        value={searchQuery}
                                ...
                                {filteredStudents.map(student => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs ${student.isVip ? 'bg-amber-100 text-amber-600' : 'bg-smartBlue text-deepNavy'}`}>
                                                {student.name.charAt(0)}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="text-sm font-bold text-slate-700 group-hover:text-deepNavy transition-colors flex items-center gap-1">
                                        ...
                                        <ChevronRight size={16} className="text-slate-300 group-hover:text-deepNavy transition-transform group-hover:translate-x-1" />
                                    </button>
                                ))}
                            </motion.div>
                        )}
                ...
                <form onSubmit={handleAddClassSubmit} className="flex-1 w-full flex gap-3 relative z-10">
                    <input
                        ...
                        className="flex-1 bg-white border border-slate-200 rounded-2xl px-4 py-3.5 text-sm font-bold text-slate-700 outline-none focus:border-deepNavy transition-all shadow-sm"
                    ...
                    <button
                        ...
                        className="bg-smartBlue hover:bg-[#d0e3f0] text-deepNavy px-6 py-3.5 rounded-2xl font-black text-sm tracking-wide shadow-sm flex items-center justify-center gap-2 border border-slate-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                    >
            ...
            <div className="space-y-4 mt-6">
                <h3 className="text-sm font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 ml-2">
                    <Users size={18} className="text-deepNavy" /> Sınıflarım ({regularClasses.length})
                </h3>
                        ...
                        {regularClasses.map(cls => (
                            <motion.div key={cls.id} whileHover={{ y: -4, scale: 1.01 }} onClick={() => onOpenClass(cls)} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm hover:shadow-float transition-all cursor-pointer flex flex-col justify-between min-h-[140px] group">
                                <div className="flex justify-between items-start">
                                    <div className="p-3 bg-smartBlue text-deepNavy rounded-xl group-hover:bg-deepNavy group-hover:text-white transition-colors">
                                ...
                                <div className="mt-4">
                                    <h4 className="font-black text-slate-800 text-lg group-hover:text-deepNavy transition-colors truncate">
                                        {cls.className}
                                    </h4>
