...
const LoginScreen = ({ onStudentLogin, onTeacherLogin, deferredPrompt, isStandalone }) => {
    ...
    useEffect(() => {
        ...
        // Arka plan rengini aydınlık (Smart Home) yap
        ...
        if (metaThemeColor) {
            ...
            metaThemeColor.setAttribute('content', '#f4f7f9');
        } else {
            metaThemeColor = document.createElement('meta');
            metaThemeColor.name = "theme-color";
            metaThemeColor.content = "#f4f7f9";
    ...
    const particlesOptions = {
        ...
        particles: {
            color: { value: ["#00b4d8", "#0c3d64", "#d4af37", "#94a3b8"] },
            links: { enable: false },
            move: {
                direction: "none",
                enable: true,
    ...
    return (
        <div className="login-scene">
                ...
                <AnimatePresence mode="wait">
                    {authView === 'selection' && (
                        <motion.div key="selection" variants={containerVariants} initial="hidden" animate="show" exit={{ opacity: 0, x: -30, transition: { duration: 0.15 } }} className="login-btns flex flex-col items-center w-full gap-2.5">
                            <motion.button variants={itemVariants} whileHover={{ scale: 1.01, y: -1 }} whileTap={{ scale: 0.98 }} onClick={() => setAuthView('student-login')} className="lbtn lbtn-s w-full p-3.5 md:p-4 rounded-[20px]">
                                <div className="liw liw-s w-9 h-9 text-base"><User size={16}/></div>
                                <div className="lbl">
                            ...
                            <motion.button variants={itemVariants} whileHover={{ scale: 1.01, y: -1 }} whileTap={{ scale: 0.98 }} onClick={() => setAuthView('vip-login')} className="lbtn lbtn-v w-full p-3.5 md:p-4 rounded-[20px]">
                                <VipParticles />
                                <div className="liw liw-v w-9 h-9 text-base"><Crown size={16}/></div>
                                <div className="lbl">
                            ...
                            <motion.button variants={itemVariants} whileHover={{ scale: 1.01, y: -1 }} whileTap={{ scale: 0.98 }} onClick={() => setAuthView('teacher-login')} className="lbtn lbtn-a w-full p-3.5 md:p-4 rounded-[20px]">
                                <div className="liw liw-a w-9 h-9 text-base"><Briefcase size={16}/></div>
                                <div className="lbl">
                            ...
                            {!isStandalone && (deferredPrompt || isIos) && (
                                <motion.button 
                                    ...
                                    className="pwa-download-btn mt-4 w-full !bg-deepNavy !text-white !border-none !shadow-smart-shadow"
                                >
                            ...
                            <motion.div variants={itemVariants} className="lquote quote-gradient-anim text-[12px] md:text-[13px] mt-6 font-black tracking-wide">
                                <span className="lqm text-[16px] text-deepNavy">"</span> Eğitim, dünyayı değiştirmek için en güçlü silahtır. <span className="lqm text-[16px] text-deepNavy">"</span>
                            </motion.div>
                        </motion.div>
                    )}
                        ...
                        <motion.div key="student-form" variants={containerVariants} initial="hidden" animate="show" exit={{ opacity: 0, x: 30, transition: { duration: 0.15 } }} className="login-btns flex flex-col items-center w-full">
                            ...
                            <motion.div variants={itemVariants} className={`p-6 md:p-8 rounded-[2rem] w-full relative overflow-hidden ${authView === 'vip-login' ? 'form-vip-card' : 'form-glass-card'}`}>
                                ...
                                <div className="mb-5 text-center">
                                    <h2 className={`text-base md:text-xl font-black tracking-wide ${authView === 'vip-login' ? 'real-gold-text' : 'text-deepNavy'}`}>
                                        {authView === 'vip-login' ? 'ÖZEL DERS GİRİŞİ' : 'ÖĞRENCİ GİRİŞİ'}
                                    </h2>
                                    <p className="text-[10px] md:text-xs text-softText mt-1 font-medium">
                                    ...
                                    <AnimatePresence>
                                        {showForgotMsg && (
                                            <motion.div 
                                                ...
                                                className="mt-2 w-full p-2.5 bg-smartBlue border border-accentTeal/20 rounded-xl text-deepNavy text-[10px] text-center"
                                            >
                                ...
                                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={handleStudentLoginSubmit} className={`lbtn w-full flex items-center justify-center rounded-[20px] p-3.5 ${authView === 'vip-login' ? 'real-gold-bg' : 'bg-deepNavy hover:opacity-90 shadow-smart-shadow'}`} style={{marginTop: '18px', border: 'none'}}>
                        ...
                        <motion.div key="teacher-form" variants={containerVariants} initial="hidden" animate="show" exit={{ opacity: 0, y: 30, transition: { duration: 0.15 } }} className="login-btns flex flex-col items-center w-full">
                            ...
                            <motion.div variants={itemVariants} className="p-6 md:p-8 rounded-[2rem] w-full form-glass-card">
                                <div className="mb-4 text-center">
                                    <h2 className="text-base md:text-lg font-black tracking-widest text-accentTeal">YÖNETİCİ GİRİŞİ</h2>
                                    <p className="text-[10px] text-softText mt-1">Öğretmen PIN kodunu girin</p>
                                </div>
                                ...
                                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => onTeacherLogin(pin)} className="lbtn lbtn-a w-full flex items-center justify-center rounded-[20px] mt-5 p-3.5" style={{background: '#0c3d64', border: 'none', boxShadow: '0 10px 30px -10px rgba(12, 61, 100, 0.3)'}}>
                                    <span style={{color: '#ffffff', fontSize: '13px', fontWeight: '900'}}>SİSTEME GİR</span>
                                </motion.button>
                            </motion.div>
                        </motion.div>
                    )}
            ...
            {/* 🔥 DIKDÖRTGEN DIKEY TELEFON TASARIMLI SİHİRBAZ MODALI (Aydınlık Tema) */}
            <AnimatePresence>
                {showIosModal && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[99999] flex items-center justify-center p-4">
                        <motion.div 
                            ...
                            className="bg-white border-2 border-slate-100 p-4 md:p-6 rounded-[2.5rem] w-full max-w-[340px] min-h-[600px] text-center shadow-smart-shadow flex flex-col justify-between relative overflow-hidden"
                            onClick={e => e.stopPropagation()}
                        >
                            <button onClick={() => setShowIosModal(false)} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-deepNavy rounded-full bg-slate-100 transition-colors z-50"><X size={15}/></button>
                            ...
                            <div className="mt-1">
                                ...
                                <h3 className="text-sm md:text-base font-black text-deepNavy uppercase tracking-wider">{iosStepsData[iosStep - 1].title}</h3>
                                <p className="text-softText text-[10px] md:text-[11px] mt-1 px-1 font-medium leading-relaxed min-h-[40px]">{iosStepsData[iosStep - 1].desc}</p>
                            </div>

                            <div className="my-2 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center justify-center h-80 overflow-hidden relative shadow-inner">
                                <img 
                            ...
                            <div className="mb-1">
                                <div className="flex justify-center gap-1.5 mb-2.5">
                                    {iosStepsData.map((step) => (
                                        <div key={step.id} className={`h-1.5 rounded-full transition-all duration-300 ${iosStep === step.id ? 'w-5 bg-deepNavy' : 'w-1.5 bg-slate-200'}`} />
                                    ))}
                                </div>
                                ...
                                <div className="flex gap-2">
                                    {iosStep > 1 ? (
                                        <button 
                                            ...
                                            className="flex-1 bg-slate-100 hover:bg-slate-200 text-deepNavy font-bold py-3 rounded-xl text-xs flex items-center justify-center gap-1 transition-colors border border-slate-200"
                                        >
                                        ...
                                        <button 
                                            ...
                                            className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-400 hover:text-deepNavy font-bold py-3 rounded-xl text-xs transition-colors border border-slate-200"
                                        >
                                            KAPAT
                                        </button>
                                    )}
                                    ...
                                    {iosStep < 4 ? (
                                        <button 
                                            onClick={() => setIosStep(iosStep + 1)}
                                            className="flex-1 bg-deepNavy hover:opacity-90 text-white font-black py-3 rounded-xl text-xs flex items-center justify-center gap-1 transition-all shadow-smart-shadow"
                                        >
                                        ...
                                        <button 
                                            onClick={() => setShowIosModal(false)}
                                            className="flex-1 bg-accentTeal hover:opacity-90 text-white font-black py-3 rounded-xl text-xs transition-all shadow-md uppercase tracking-wider"
                                        >
                                            HAZIRIM 🎉
                                        </button>
                                    )}
