                            ...
                            {(provided) => (
                                <div {...provided.droppableProps} ref={provided.innerRef}>
                                    {localCurriculum.map((topic, index) => {
                                        ...
                                        const unitStateClass = tProgress === 100 ? 'done' : (tProgress > 0 ? 'active' : 'locked');
                                        const completedSubCount = topic.subTopics ? topic.subTopics.filter(st => st.isCompleted).length : 0;
                                        const totalSubCount = topic.subTopics ? topic.subTopics.length : 0;
                                            ...
                                            <Draggable key={topic.id} draggableId={topic.id} index={index} isDragDisabled={!isTeacherMode}>
                                                {(provided, snapshot) => (
                                                    <article 
                                                        ...
                                                        className={`unit ${unitStateClass} group/topic ${snapshot.isDragging ? 'shadow-2xl z-50 bg-white' : ''}`}
                                                    >
                                                        <div className="unit-head">
                                                             <div className="unit-title">
                                                                {isEditingThisTopic ? (
                                                                     <div className="flex items-center gap-1.5 w-full max-w-md mb-1">
                                                                     ...
                                                                     <div className="flex items-center gap-2">
                                                                        {isTeacherMode && <div {...provided.dragHandleProps} className="cursor-grab text-slate-400 hover:text-primary"><GripVertical size={16} /></div>}
                                                                        <h2 onClick={() => { if(totalSubCount === 0) toggleTopic(topic.id); }} className={totalSubCount === 0 ? "cursor-pointer hover:text-primary transition-colors" : ""}>{topic.title}</h2>
                                                                        {isTeacherMode && (
                                                                ...
                                                                {totalSubCount > 0 ? <p>{completedSubCount}/{totalSubCount} alt konu tamamlandı</p> : <p>{topic.isCompleted ? 'Tamamlandı' : 'Bekliyor'}</p>}
                                                             </div>
                                                             <div className="unit-percent">%{tProgress}</div>
                                                        </div>

                                                        {totalSubCount > 0 && (
                                                            <div className="unit-progress">
                                                                 <div className="unit-bar" style={{"--value": `${tProgress}%`}}><span></span></div>
                                                                 <div className="unit-count">{completedSubCount}/{totalSubCount}</div>
                                                            </div>
                                                        )}

                                                        <Droppable droppableId={topic.id} type="subtopic">
                                                             {(provided) => (
                                                                  <div {...provided.droppableProps} ref={provided.innerRef} className="topic-list">
                                                                               ...
                                                                               <Draggable key={sub.id} draggableId={sub.id} index={subIndex} isDragDisabled={!isTeacherMode}>
                                                                                   {(provided, snapshot) => (
                                                                                        <div 
                                                                                            ...
                                                                                            style={getItemStyle(snapshot.isDragging, provided.draggableProps.style)}
                                                                                            className={`topic group/sub ${topicStateClass} ${snapshot.isDragging ? 'shadow-md bg-white z-50 rounded-lg p-2' : ''}`}
                                                                                        >
                                                                                            <div className="topic-check" onClick={() => toggleSubTopic(topic.id, sub.id)}></div>
                                                                                            {isEditingThisSub ? (
                                                                                                ...
                                                                                                <div className="flex items-center justify-between w-full">
                                                                                                    ...
                                                                                                    <div className="flex items-center gap-2">
                                                                                                        <span className="topic-state">{stateText}</span>
                                                                                                        {isTeacherMode && (
                                                                      ...
                                                                      {isTeacherMode && (
                                                                          <div className="flex items-center gap-2 mt-1 opacity-50 focus-within:opacity-100 transition-opacity">
