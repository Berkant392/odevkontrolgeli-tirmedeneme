...
const Sidebar = ({ 
    ...
    const teacherMenu = [
        { id: 'home', label: 'Sınıflar', icon: Home, action: () => setActiveTab('homework') },
        { id: 'library', label: 'Kütüphane', icon: BookOpen, action: handleOpenLibrary },
    ];

    const studentMenu = [
        { id: 'home', label: 'Ana Sayfa', icon: Home },
        { id: 'homework', label: 'Ödev Takip', icon: ClipboardList },
        { id: 'trials', label: 'Net Takibi', icon: Target },
        { id: 'curriculum', label: 'Müfredat Takip', icon: Map },
    ];
        ...
        <div className={`h-full flex flex-col justify-between ${currentUserRole === 'vip-student' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border-r shadow-xl relative`}>
            ...
            <div className="flex-1 py-6 flex flex-col gap-2 overflow-y-auto overflow-x-hidden px-3">
                {menuItems.map((item) => {
                    ...
                    // Eğer ana sayfa değilse activeTab kontrolü yap
                    const isActive = currentUserRole === 'teacher' 
                        ? (activeTab === item.id || (!item.action && activeTab !== 'library')) // Öğretmen anasayfa
                        : activeTab === item.id;
