...
import LiveClassroom from './components/views/LiveClassroom';
import LiveSessionManager from './components/views/LiveSessionManager';
...
const App = () => {
    ...
    useEffect(() => { const handleResize = () => setIsMobile(window.innerWidth < 1024); window.addEventListener('resize', handleResize); return () => window.removeEventListener('resize', handleResize); }, []);

    const [newStudentName, setNewStudentName] = useState("");
    const [newStudentPhone, setNewStudentPhone] = useState("");

    // 🎥 CANLI DERS VE GÖRÜNTÜLÜ SINIF STATE'LERİ
    const [liveSessions, setLiveSessions] = useState([]);
    const [activeLiveSession, setActiveLiveSession] = useState(null);
    const [isJitsiLoaded, setIsJitsiLoaded] = useState(false);

    // Jitsi Meet External API'sini Dinamik Olarak Yükleyen Effect
    useEffect(() => {
    ...
    // Firestore Canlı Ders Dinleyicisi
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // Eğer aktif bir oturumumuz varsa ve o artık Firestore'da yoksa veya güncellendiyse senkronize et
            if (activeLiveSession) {
                const current = sessions.find(s => s.id === activeLiveSession.id);
                if (!current) {
                    setActiveLiveSession(null);
                    if (view === 'live-classroom') setView('home');
                } else {
                    setActiveLiveSession(current);
                }
            }
        }, (err) => {
            console.error("Canlı ders dinleme hatası:", err);
        });
        return () => unsubscribe();
    }, [activeLiveSession, view]);

    const startLiveSession = async (cls) => {
        if (!cls) return;
        try {
            const roomId = `berkant-vip-${cls.id}-${Math.random().toString(36).substring(2, 9)}`;
            const newSession = {
                classId: cls.id,
            ...
            setActiveLiveSession(createdSession);
            setView('live-classroom');
            showAlert('success', 'Canlı Ders Başlatıldı', `${cls.className} için canlı ders odası başarıyla açıldı.`);
        } catch (e) {
            console.error("Canlı ders başlatılamadı:", e);
            showAlert('error', 'Hata', 'Canlı ders odası oluşturulurken hata meydana geldi.');
        }
    };
    ...
    const endLiveSession = async (sessionId) => {
        ...
        try {
            ...
            setActiveLiveSession(null);
            setView('home');
            showAlert('info', 'Canlı Ders Sonlandırıldı', 'Ders başarıyla tamamlandı.');
        } catch (e) {
            console.error("Canlı ders sonlandırılamadı:", e);
        }
    };
        ...
        <div className={`flex h-screen overflow-hidden ${currentUserRole === 'vip-student' ? 'bg-slate-900' : 'bg-lightBg'}`}>
            ...
            <div className="flex-1 overflow-y-auto pb-24 md:pb-32 transition-colors duration-1000 relative">
                ...
                <AnimatePresence mode="wait">
                    ...
                    {isTeacherMode && view === 'home' && activeTab === 'vip-classes' && <VipDashboard vipClasses={vipClasses} onOpenClass={openClass} onNewVipClass={handleAddVip} startLiveSession={startLiveSession} />}
                    {isTeacherMode && view === 'home' && activeTab === 'live-sessions' && <LiveSessionManager classes={classes} isTeacherMode={true} showAlert={showAlert} startLiveSession={startLiveSession} joinLiveSession={joinLiveSession} />}
                    ...
                    {!isTeacherMode && view === 'home' && activeTab === 'home' && <StudentDashboard classes={classes} currentUserRole={currentUserRole} loggedInStudent={loggedInStudent} onOpenClass={openClass} setView={setView} countdownConfig={countdownConfig} liveSessions={liveSessions} joinLiveSession={joinLiveSession} selectedClass={selectedClass} />}
                    {!isTeacherMode && view === 'home' && activeTab === 'live-sessions' && <LiveSessionManager classes={classes} isTeacherMode={false} showAlert={showAlert} loggedInStudent={loggedInStudent} liveSessions={liveSessions} joinLiveSession={joinLiveSession} />}
