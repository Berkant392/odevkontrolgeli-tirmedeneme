...
import AddClassView from './components/views/AddClassView';
import AddVipView from './components/views/AddVipView';
    ...
    return (
        <div className={`flex h-screen overflow-hidden ${currentUserRole === 'vip-student' ? 'bg-slate-900' : 'bg-lightBg'}`}>
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden">
                <div className="absolute inset-0 overflow-y-auto pb-24 md:pb-32 transition-colors duration-1000">
                    ...
                    <header className={`no-print relative z-20 transition-all duration-500 ${currentUserRole === 'vip-student' ? 'bg-slate-800/90 border-b border-slate-700 shadow-md' : 'bg-white border-b border-slate-200 shadow-sm'} sticky top-0`}>
                        ...
                        <div className="flex items-center gap-1.5 justify-end relative">
                            {(currentUserRole === 'student' || currentUserRole === 'vip-student' || currentUserRole === 'teacher') && (
                                <div className="relative">
                                    <button onClick={() => setShowNotifications(!showNotifications)} className={`p-1.5 md:p-2 rounded-xl transition-colors hover-lift relative ${currentUserRole === 'vip-student' ? 'text-slate-300 hover:text-vipGold bg-slate-700 border border-slate-600 shadow-sm' : 'text-slate-500 hover:text-brandPurple bg-white shadow-sm border border-slate-200'}`} title="Bildirimler">
                                        <Bell size={18} />
                                        {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-rose-500 rounded-full flex items-center justify-center text-[9px] font-bold text-white border-2 border-white">{unreadCount > 9 ? '9+' : unreadCount}</span>}
                                    </button>
            ...
            <main className="max-w-7xl mx-auto px-2.5 mt-5 no-print relative z-10">
                {((currentUserRole === 'teacher' && view === 'home') || ((currentUserRole === 'student' || currentUserRole === 'vip-student') && activeTab === 'home')) && (
                ...
                <AnimatePresence mode="wait">
                    {isTeacherMode && view === 'home' && activeTab === 'home' && <TeacherDashboard regularClasses={regularClasses} vipClasses={vipClasses} onOpenClass={openClass} />}
                    {isTeacherMode && view === 'home' && activeTab === 'add-class' && <AddClassView onAddClass={handleAddClass} />}
                    {isTeacherMode && view === 'home' && activeTab === 'add-vip' && <AddVipView onAddVip={handleAddVip} />}
