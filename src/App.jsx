...
const App = () => {
    ...
    // 🔥 GÜNCELLEME: ULTRA-PREMIUM KURUMSAL LOGOLU VE METİN KAYMASI ENGELLENMİŞ PDF ÇIKTI MOTORU + DENEME TAKİBİ
    const handlePrintStudentReport = async (cls, student) => { 
        ...
        // Firebase'den öğrencinin deneme verilerini çek
        let studentTrials = [];
        try {
            const { getDocs, query: fbQuery, where: fbWhere, orderBy: fbOrderBy } = await import('firebase/firestore');
            const trialsQuery = fbQuery(collection(db, 'trials'), fbWhere('studentId', '==', student.id));
        ...
        } catch (e) { console.error("Deneme verileri çekilemedi:", e); } 
        ...
        if (studentTrials.length > 0) {
            ...
            <div style="margin-top: 30px; page-break-inside: avoid;">
                <h3 style="font-size: 16px; font-weight: 800; color: #0f172a; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px; border-bottom: 2px solid #6366f1; padding-bottom: 8px;">
                    📊 Deneme Sonuçları Özeti
                </h3>
                <div style="display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 120px; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 12px; text-align: center;">
                        <div style="font-size: 22px; font-weight: 900; color: #15803d;">${studentTrials.length}</div>
                        <div style="font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 1px;">Çözülen Deneme</div>
                    </div>
                    <div style="flex: 1; min-width: 120px; background: #fefce8; border: 1px solid #fde68a; border-radius: 12px; padding: 12px; text-align: center;">
                        <div style="font-size: 22px; font-weight: 900; color: #b45309;">${trialAvg}</div>
                        <div style="font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 1px;">Ortalama Net</div>
                    </div>
                    <div style="flex: 1; min-width: 120px; background: #fff1f2; border: 1px solid #fecdd3; border-radius: 12px; padding: 12px; text-align: center;">
                        <div style="font-size: 22px; font-weight: 900; color: #be123c;">${trialMax}</div>
                        <div style="font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 1px;">En Yüksek Net</div>
                    </div>
                </div>
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width: 30%;">Ders Bazlı Netler</th>
            ...
            studentTrials.forEach(trial => {
                ...
                if (trial.details) {
                    if (trial.type === 'TYT') {
                        detailStr = 'Tür: ' + (trial.details.turkce?.toFixed(1) || '0') + ' | Mat: ' + (trial.details.mat?.toFixed(1) || '0') + ' | Sos: ' + (trial.details.sosyal?.toFixed(1) || '0') + ' | Fen: ' + (trial.details.fen?.toFixed(1) || '0');
                    } else if (trial.type === 'AYT') {
                        detailStr = 'Edb: ' + (trial.details.edebiyat?.toFixed(1) || '0') + ' | Mat: ' + (trial.details.matAyt?.toFixed(1) || '0') + ' | Fen: ' + (trial.details.fenAyt?.toFixed(1) || '0') + ' | Sos: ' + (trial.details.sosyalAyt?.toFixed(1) || '0');
                    } else if (trial.type === 'YDT') {
                        detailStr = 'YDT: ' + (trial.details.ydt?.toFixed(1) || '0');
                    }
                }
                html += `
                        <tr>
                            ...
                            <td><span class="badge ${trial.type === 'TYT' ? 'badge-done' : trial.type === 'AYT' ? 'badge-missing' : 'badge-exempt'}">${trial.type}</span></td>
                            <td style="font-size: 11px;">${detailStr}</td>
                            <td><strong style="color: #6366f1; font-size: 15px;">${trial.totalNet}</strong></td>
                        </tr>`;
            });
        ...
        <div className={`min-h-screen pb-24 md:pb-32 relative transition-colors duration-1000 ${currentUserRole === 'vip-student' ? 'bg-slate-900' : 'bg-lightBg'}`}>
            ...
            <main className="max-w-7xl mx-auto px-2.5 mt-5 no-print relative z-10">
                <AnimatePresence mode="wait">
                    ...
                    {!isTeacherMode && view === 'home' && <StudentDashboard classes={classes} currentUserRole={currentUserRole} onOpenClass={openClass} setView={setView} />}
