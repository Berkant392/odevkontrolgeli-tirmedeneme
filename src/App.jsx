...
const App = () => {
    ...
    const studentNotifications = React.useMemo(() => {
        ...
        return combined.slice(0, 10); // Son 10
                            ...
                            {(currentUserRole === 'student' || currentUserRole === 'vip-student') && (
                                <div className="relative">
                                    <button onClick={() => setShowNotifications(!showNotifications)} className={`p-1.5 rounded-full transition-colors hover-lift relative ${currentUserRole === 'vip-student' ? 'text-slate-300 hover:text-vipGold bg-slate-700 border border-slate-600 shadow-sm' : 'text-slate-500 hover:text-brandPurple bg-white shadow-sm border border-slate-200'}`} title="Bildirimler">
                                        <Bell size={16}/>
                                        {studentNotifications.length > 0 && <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-500 rounded-full flex items-center justify-center text-[8px] font-bold text-white border-2 border-white">{studentNotifications.length}</span>}
                                    </button>
                                                ...
                                                <div className="max-h-80 overflow-y-auto">
                                                    {studentNotifications.length === 0 ? (
                                                        ...
                                                        studentNotifications.map((n, idx) => (
                                                            <div key={n.id || idx} className="p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors flex gap-3">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="text-[11px] text-slate-500 leading-relaxed">{n.text}</div>
